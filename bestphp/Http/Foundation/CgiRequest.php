...
class CgiRequest
{
    ...
    {
        $stream = Container::getInstance()->get('stream');
        $this->input = $stream->open('php://input', 'r');
        return $this;
    }
    ...
    protected function uploadedFileFactory($uploadedFiles)
    {
        ...
        foreach ($files as $id => $info) {
            if (count($info) === count($info, 1)) {
                $files[$id] = Container::getInstance()['uploaded_file']->make($info);
            } else {
                foreach ($info as $key => $value) {
                    $files[$id][$key] = Container::getInstance()['uploaded_file']->make($value);
                }
            }
        }
